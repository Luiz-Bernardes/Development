...
# form view
.nested-fields.js-parent-solicitation_material
  .row.skn-inline-fields.item-parent
    .col-md-3
      .hidden
        = f.input :value, as: :string , input_html: { class: 'js-total-value' }
      = f.input :formatted_value, as: :string , input_html: { class: 'js-price-format js-transform-value input_add', data: { 'transform-target' => '.js-total-value'} , value: (f.object.value || 0) }

javascript:
  $('body').on('keyup', '.js-transform-value', function(){
    var $this = $(this);
    var result_price = allowOnlyNumbers($(this).val());
    var $parent_item = $this.closest('.item-parent');
    $parent_item.find($this.data('transform-target')).val(result_price);
  });

  # OR without nested fields
  $('body').on('keyup', '.js-transform-value', function(){
    var result_price = allowOnlyNumbers($(this).val());
    $($(this).data('transform-target')).val(result_price);
  });